...
def test_kills(config_path):
    ...
    player_input = {"id": white_pawn.id, "action": black_pawn.position}

    # Handle game
    engine.handle_game(player="white", player_input=player_input, game_state=game_state)
...
def test_player_is_in_check(config_path, player):
    ...
    if player == "white":
        # Spawn white king
        engine.spawn_piece(piece_nr=6, position=(4, 4))

        # Spawn black rook
        engine.spawn_piece(piece_nr=8, position=(4, 6))
    else:
        # Spawn black king
        engine.spawn_piece(piece_nr=12, position=(4, 4))

        # Spawn white rook
        engine.spawn_piece(piece_nr=2, position=(4, 6))
